...
def plot_graphs(input_file_info, cache_params, pp):
    ...
    for i in range(0,14):
        lru = data.loc[i,'LRU']
        data.loc[i,'DIP0.015625'] = float(data.loc[i,'DIP0.015625']) / lru 
        data.loc[i,'DIP0.03125'] = float(data.loc[i,'DIP0.03125']) / lru
        data.loc[i,'DIP0.0625'] = float(data.loc[i,'DIP0.0625']) / lru
